...
describe('CastMemberSequelizeRepository Integration Tests', () => {
  ...
  it('should insert many cast member', async () => {
    const castMembers = [
    ...
    const castMemberCreated = await repository.findByIds(
      castMembers.map((castMember) => castMember.cast_member_id),
    );

    expect(
      castMemberCreated!.map((castMember) => castMember.toJSON()),
    ).toStrictEqual(
      castMembers
        .map((castMember) => castMember.toJSON())
        .sort((a, b) => a.cast_member_id.localeCompare(b.cast_member_id)),
    );
  });
